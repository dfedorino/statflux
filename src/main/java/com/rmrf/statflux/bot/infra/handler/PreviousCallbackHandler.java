...
@Slf4j
public class PreviousCallbackHandler extends AbstractStatsCallbackHandler {
    ...
    public void handle(TelegramBotContext ctx, Consumer<TelegramBotContext> next) {
        ...
        if (!ctx.update().hasCallbackQuery() || !"prev".equals(callbackQuery.getData())) {
            next.accept(ctx);
            return;
        }
        log.debug("callback 'prev' handling");

        MaybeInaccessibleMessage message = callbackQuery.getMessage();
        var statsResult = serviceLayer.getPreviousVideos(message.getChatId(), (long) message.getMessageId());
        if (statsResult.isFailure()) {
            handleFailure(ctx, l10n.callbackQueries.prevError);
            return;
        }
